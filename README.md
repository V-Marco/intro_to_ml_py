...
- Домашнее задание 2: Линейная регрессия, классификация, решающие деревья. (19 января – 1 марта, 23:59):
  - [Notebook с заданием](https://github.com/V-Marco/intro_to_ml_py/blob/master/hw2/hw_2.ipynb)
  - [Данные к заданию](https://github.com/V-Marco/intro_to_ml_py/blob/master/hw1/FISH.csv)
  
__Проекты:__

Цель выполнения проекта – получить опыт постановки и решения исследовательской задачи на реальных данных. Проект состоит из следующих этапов:

  1. Постановка исследовательского вопроса и поиск данных для ответа на него. На этом этапе можно пойти по одному из двух направлений: либо сначала сформулировать исследовательский вопрос, например, по теме, явлению или ситуации, которые заинтересовали Вас, а затем найти необходимые данные; либо сначала найти интересный набор данных, провести его предварительный анализ (построить диаграммы рассеяния, рассчитать корреляционную матрицу и т.д.) и затем поставить исследовательский вопрос.
  
     Разрешается использовать данные из любого публичного источника, который можно найти в поисковой системе. Примерами ресурсов с большой базой наборов данных являются [kaggle](https://www.kaggle.com) и [UCI](https://archive.ics.uci.edu/ml/index.php). Рекомендуется брать такой набор данных, в котором представлены как непрерывные, так и категориальные переменные. 
 
  2. Формулировка гипотезы и механизма, её объясняющего. Постановка задачи регрессии или классификации для проверки гипотезы. Выбор метода машинного обучения для решения поставленной задачи. Можно использовать любой метод, который был изучен на занятиях. Также разрешается использование методов, которые не освещались на занятиях (например, логистическая регрессия), но только если Вы сможете объяснить, как работает данный метод. 
  
  3. Решение задачи и презентация результатов. Итогом проекта являются: тетрадка Jupyter Notebook с кодом, который использовался для решения задачи, и презентация результатов. Тетрадка отправляется преподавателю после выступления с презентацией. В презентацию должны войти: 
  
  - Исследовательский вопрос, гипотеза, механизм. 
  - Краткое описание данных и графики из предварительного анализа.
  - Постановка задачи регрессии и классификации, методы машинного обучения для решения этой задачи. Один или несколько показателей качества оценки модели. 
  - Полученные результаты: подтверждается или опровергается гипотеза (если гипотеза опровергается, то необходимо выдвинуть предположение, почему это так), ответ на исследовательский вопрос (если возможно его дать). 
  
     Несколько комментариев. Если используется линейная регрессия, необходимо привести интерпретацию коэффициентов (то есть как использованные в модели признаки влияют на зависимую переменную и возможное объяснение именно такого влияния). Если используется случайный лес или kNN, необходимо пояснить, почему было выбрано именно такое число деревьев/соседей. 
     
     Также важное замечание: разрешается использовать фрагменты кода, найденные в Интернете, но в таком случае необходимо обязательно указать ссылку на источник. В презентации ссылку на источник следует привести для всех сторонних (то есть созданных не Вами) материалов: данные, графики, изображения и проч. Ссылка на источник указывается в виде комментария в коде или внизу слайда мелким шрифтом. 
  
     Презентации будут защищаться в марте, по одной-две презентации на одном занятии. Презенатция не должна быть слишком длинной или перегруженной: лучше наполнить её графиками, чем текстом. Время на выступление: не более 10 минут.
